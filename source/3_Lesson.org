...
- Can you remix the code above to write a happy/sad program? If the user pushes button A, the =HAPPY= image should be loaded onto the screen. If they push B, =SAD= should be shown. If they push nothing, the image should change to =ASLEEP=. 
** Learn It
:PROPERTIES:
:HTML_CONTAINER_CLASS: learn
:END:
...
while True:
    ...
    display.scroll(currentNumber)
#+end_src
- This program is ok, but we could use some more =If= lines to make it better. 
- I only want the program to show the numbers 0-9, but at the moment it'll give me negative numbers, and will count beyond 9.
- By adding another =If= statement, we could say something like 'if the currentNumber goes above 9, set it back to 9', perhaps?
...
- *Silver:* Modify the code to start at 1, and show the 6 times table instead. Upload your code to BourneToLearn.com 
- *Gold:* Change the code back to a counting up and down program, but modify the code so that the user can't make the numbers go above 9 or below 0. 
- *Platinum:* Write an app to count down from 10 to 0, then show a smiley face. 
